...
#pragma GCC diagnostic pop
  ...
  {
    ftl::Matrix<TypeParam> expected =
        CreateInitializedMatrix<TypeParam>(3, 3, 2);
  ...
  TYPED_TEST(MatrixTest, AdditionOperatorValid)
  {
    ftl::Matrix<TypeParam> expected =
        CreateInitializedMatrix<TypeParam>(3, 3, 2);
    AssertMatricesEqual(this->matrix + ftl::Matrix<TypeParam>(3, 3, 1),
                        expected);
  }
  ...
  TYPED_TEST(MatrixTest, BeginEnd)
  {
    auto beg = this->matrix.begin();
    auto end = this->matrix.end();
    EXPECT_NE(beg, end);
    EXPECT_NE(*beg, *(end - 1));
    EXPECT_EQ(*beg, *(end - this->matrix.size()));
  }
